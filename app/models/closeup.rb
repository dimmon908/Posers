class Closeup < ActiveRecord::Base
  ...
  def url
    ...
    unless self.filename.nil?
      AWS::S3::S3Object.url_for(image_file_name, 'xposers_closeups', :expires => doomsday)
    else
      names = image_file_name.split(".")
      extension = names.pop
      file = "#{self.id}_#{names.join(".")}_normal.#{extension}"
      return AWS::S3::S3Object.url_for(file, 'xposers_closeups', :expires => doomsday)
    end
  end
end
